...
const ManageAcceptedEvents = () => {
    let serial = 0;
    const [getEvent, setGetEvent] = useState([]);
    const mount= async()=>{
        const res = await axios.get('http://localhost:8000/api/admin/manageAcceptedEvent');
    ...
    const deleteEvent = async(id)=>{
        const res = await axios.get(`http://localhost:8000/api/admin/manageAcceptedEvent/delete/${id}`);
        mount();
                        ...
                        <div className="col-sm-12 offset-sm-0" style={{ "marginTop" :"0px"}}>
                            <div className="card">
                                <div className="card-header" style={{ "padding" :"5px"}}>
                                    <h4 className="card-title"> Manage Events List</h4>
                                </div>
...
export default ManageAcceptedEvents;