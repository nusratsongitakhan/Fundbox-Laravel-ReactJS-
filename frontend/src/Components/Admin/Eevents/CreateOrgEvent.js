...
const CreateOrgEvent = () => {
    ...
    const addEvent = async (e) => {
        e.preventDefault();
        const org_id = event.org_id.toString();
        ...
        const event_amount =event.event_amount.toString();
        const date  =event.date.toString();
        const event_phone = event.event_phone.toString();
        const status = event.status.toString();
        // const image = selectedFile;
        const res = await axios.post('http://localhost:8000/api/admin/createOrgEvent', 
        { 
            org_id:org_id,event_name: event_name,event_details: event_details,event_category: event_category,event_amount: event_amount,date : date,event_phone : event_phone,status:status
        },{
            headers: { 
    ...
    const getOrgList= async()=>{
        const res = await axios.get(`http://localhost:8000/api/admin/getAllOrg`);
        console.log(res.data);
        if (res.status === 200) {
            setGetOrg(res.data)
        }
                        ...
                        <div className="col-sm-12 offset-sm-0" style={{ "marginTop" :"0px"}}>
                            <div className="card">
                                <div className="card-header">
                                    <h4 className="card-title">Create Organization Event </h4>
                                </div>
                                            ...
                                            <div className="row">
                                                ...
                                                <div className="col-12 col-sm-12 col-lg-12" style={{ "marginTop" :"10px"}}>
                                                    ...
                                                    <fieldset className="form-group">
                                                        <select name="org_id" className="form-control" onChange={handleInput} required>
